    ...
    while True:
        ...
        try:
            ...
            if page_type == 's_collection':
                product_items = soup.find_all('article', class_='product-grid-item')
            else:
...
def download_and_rename(session, download_link, artist_name, title, folder_name):
    try:
        ...
        filename = f"{sanitize_filename(artist_name)} - {sanitize_filename(title)}.jpg"
...
def process_page(session, page_url):
    try:
        ...
        for idx, info in enumerate(artwork_info_list, 1):
            print(f"\nXử lý tác phẩm {idx}/{len(artwork_info_list)}: {info['url']}")
            data_id = info['data_id']
            title = info['title']

            # Nếu là trang nghệ sĩ, dùng folder_name làm tên nghệ sĩ; nếu không, dùng "Various Artists"
            artist_name = folder_name if page_type == 'artist' else "Various Artists"

            # Gửi yêu cầu AJAX để lấy đường dẫn tải xuống
